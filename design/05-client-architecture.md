...
**The shell runs a node per network, since 2026-09-10.** It held one — "the node running for the
open network" — and switching networks dropped the task for the one being left. `App` now holds a
`kols_node::nodes::Nodes` and the network in view, and every path that used to start a node
reconciles instead. Three consequences worth having written down:

- **Opening a network is no longer a restart.** It raises one node to hot and lowers another to
  warm, and tears down neither, because both tiers are running nodes. A member switching between
  two networks used to lose the connections of the one they left.
- **One thing still needs a genuine restart**, and only one: a node reads its relay set at
  startup, so designating a relay while it runs changes policy and reaches nothing. That path
  stops the node so the reconcile behind it starts a new one.
- **Shutdown stops every node and awaits them**, which the single-node version already knew to do
  for one. Aborting is a request: a claim is released when the serving future is *dropped*, so
  returning without awaiting would leave a dozen stores claimed and a dozen relay slots held.

**An event now says which network it came from.** `Event` carries no network id — it never needed
one while a single node reported — so the supervisor tags each batch with the network whose node
produced it, and the interface ignores what is not about the network it is showing. Tagged at the
supervisor rather than added to the type, so §3's vocabulary is unchanged for consumers that
still watch one network.

**And a tick, because a poll is a schedule.** `reconcile` is idempotent and decides everything
from the state it is handed, so a timer costs nothing while nothing has changed — and it is the
only thing that will ever wake a network the member set aside. Without it, cold would be the "no
node, ever" `09` §2 was just corrected away from.
...
**Three times, and the third is not a default — it is a platform.** `v0.13.2` opened a
network's window from a **synchronous** `#[tauri::command]`, and on Windows that gave a blank
white window which could not be closed, with the tray's quit doing nothing: three symptoms of
one stuck thread. A synchronous command runs its body *inline in the IPC handler*
(`ExecutionContext::Blocking` is the macro's default, and becomes `Async` only when the
function is `async`), and on Windows that handler is dispatched on the webview's own
message-loop thread. `WebviewWindowBuilder::build()` there is the deadlock Tauri documents
under *Known issues* — WebView2's controller is created asynchronously and completes through
the message loop, so the thread that has to pump it is the thread blocked waiting for it. The
remedy is the documentation's own: *use `async` commands and separate threads when creating
windows*.

**What makes this worth a paragraph rather than a fix is where it could not be caught.** Every
gate this project runs executes on Linux, where WebKitGTK creates its webview synchronously on
the calling thread and the same code is correct. So there was no failing test to find: the
container is not a weaker test of this property, it is a test of a different platform. Nor is
this the kind of thing a release build reveals, since `v0.13.1` had one window, created from
the configuration during setup, and never built one from a command at all — the feature that
introduced the trap is the feature that needed the window.

The guard is therefore a **source rule**, which is what is checkable everywhere:
`crates/kols-app/tests/window_creation.rs` asserts that no command building a window is
synchronous and that no event handler builds one, following one level of call so that a
command reaching a helper counts. It was written by reverting the fix and watching it fail,
...
- **A green suite can lie in a way a red one cannot.** The first two of these were denials
  that produced no output; the third produced no output *on the only platform the tests run
  on*. What that adds to the rule below is a second question to ask of a shell behaviour: not
  only *is this asserted against the real configuration*, but *could this assertion be true
  here and false where it ships*. Where the answer is yes, the property has to be checked as
  a rule about the source rather than as behaviour under test.
...
Command  = OpenChannel { channel_id, before: Option<Hlc>, limit }
         ...
         | FetchHistory { channel, before }                        — §5.1, `09` §4.4
...
**What it was not wired to is `open_channel`, and that was a product decision rather than
plumbing.** The window's `open_channel` took a channel and nothing else — there was no paging in
the interface at all, and `before` and `limit` had been on the command since it was written
without ever being passed, destructured away at the executor as well as at the window. Rendering
a page into an interface that cannot ask for the next one would hide history with no way to reach
it, which is a worse failure than the slowness it fixes.

**`09` §4.4 now answers it**, and answering it moved one thing back down here: the cursor type is
wrong. `before: Option<Hlc>` cannot express a page boundary, because records merge by reading and
*then* by record id — the counter is per author and device, so two members writing in the same
millisecond share a reading legitimately. A boundary landing between such a pair excludes both,
including the one never drawn, and nothing above can detect it. The cursor is the pair the merge
sorts on, and it comes from `kols-core` rather than from a second copy of the comparator, for the
reason the verdict fold asserts against `kols_core::withheld` rather than reimplementing it.

#### Built 2026-09-09, and what building it found

| Records in the channel | A settled page of 50 | Opening it whole |
|---|---|---|
| 500 | 3.6 ms | 22.8 ms |
| 2,000 | 8.2 ms | 110.2 ms |
| 4,000 | 5.9 ms | 210.3 ms |
| 8,000 | 6.1 ms | 397.7 ms |

The whole-channel column is linear at about 0.05 ms a record — **roughly five seconds at a
hundred thousand**, on a control the window re-runs every two seconds. The page column is what
O4 was for.

**It was not flat at first, and the fix for that is the more important half of this work.** A
page cost the records it drew *plus a directory listing of the channel*, which is how the store
noticed whether anything was new — 3.6 ms at five hundred records and 6.1 ms at eight thousand,
which reads as flat and is linear. `09` §4.4 now requires the tick to be bounded outright, the
counters in `Store::work` make that a test rather than a measurement, and a settled read does no
directory listings at all: 52 file reads for a page of fifty, at five hundred records and at
eight thousand alike.

**The measurement found the bottleneck the index was hiding, which is the same shape as the last
two.** Before every page, `fold_into` read *every record file in the channel and decoded it*, to
compare a count. So the index was built, paid for, and then not used: a page cost the channel
anyway. Record files are named by their id, so the directory answers *which records exist*
without opening one — and once that was fixed, a second layer of the same mistake showed up
underneath it, materialising every identifier out of SQLite to discover that none of them were
new. A count answers that.

**And it found a ceiling that lied.** The reach limit started life in the store, which silently
truncated `Window::opening(usize::MAX)` — so `kols read` would have shown the last five hundred
messages of a channel and said nothing about the rest. That is the same class of failure as the
page boundary that drops a message: the answer is wrong and nothing on screen reveals it. The
limit belongs where untrusted input arrives, and the store does what it is told.

**Twice the measurement itself was the thing that was wrong**, which is worth recording because
both readings looked plausible. The first conflated folding a five-hundred-record batch with
reading a page, and reported forty milliseconds for something that costs four. The second added
a store-level column under hand-written limits that differed from the command's — so every call
re-folded the channel, and the column measured the fold while appearing to measure the read. A
verdict being true only of the limits that produced it is a property of this design; a
measurement that varies them is measuring that property rather than the thing it asked about.
...
| API boundary (§3) | Every command against a replayed log, not a hand-built state: a grant reaches the scope it names and no further, an ordinary verb does not buy a governance-tier one, and the consent class agrees with the vocabulary's tier table | **Done**, both halves. The command half is 18 cases plus a `compile_fail` doctest for the unforgeable token. The event half gained the same pair of guards on 2026-09-07 (`tests/events.rs`) — an exhaustive `match` with no wildcard arm, and an assertion that the sample list covers every name — after §3's event list had drifted three times with nothing to catch it. The guard was watched failing before it was believed: adding a variant produces `error[E0004]`, and the first attempt proved only that `#![deny(missing_docs)]` works, because the probe had stolen a doc comment and failed earlier for an unrelated reason |
...
| Node supervision (§1, `09` §2) | Several networks live at once: the tier each is kept at, that a set-aside network is *polled* rather than abandoned, that reconciling is idempotent, and that a tier change between two running tiers is not a restart. Plus the one a suite naturally misses — **reconciling from a synchronous caller**, outside any runtime, which is what a Tauri command is | **Done** — 7 policy cases, 4 live ones running two and three real nodes, and the synchronous-caller regression. That last exists because `v0.13.0` shipped without it: `tokio::spawn` finds the runtime *entered on the calling thread* and panics where there is none, and every test reconciled inside `block_on`, so the suite agreed with the code about an assumption neither had stated. **Untested in the field**: one machine cannot show contention between many nodes, delivery to a network nobody is looking at, or claims released on close |
...
| Window creation is off the message-loop thread (§1) | That no `#[tauri::command]` which builds a window is synchronous, and that no event handler builds one. A synchronous command runs inline in the IPC handler, which on Windows is that thread, and `build()` there deadlocks — a blank window, unclosable, with the tray dead beside it | **Done** — `tests/window_creation.rs`, a rule about the source rather than behaviour under test, because every gate here runs on Linux where the same code is correct. Written after `v0.13.2` shipped it, and verified by reverting the fix |
