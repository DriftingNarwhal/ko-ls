...
**It is not perfectly flat and the reason is worth stating rather than rounding away.** A page
costs the records it draws plus one directory listing of the channel, which is how the store
notices whether anything is new without trusting a counter it maintains. That listing is about
0.0003 ms a record — some thirty milliseconds at a hundred thousand — and it buys the property
every cache here has: the index is derived, deletable, and never a second source of truth.
