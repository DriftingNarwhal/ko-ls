...
**The rule that matters is the one this section already stated**: a conversation is warm whenever
the application is running, in view or not. Everything else follows from it.

**Hot and warm are both running nodes**, and that is worth saying because "warm" sounds cheaper
than it is. The distinction this section drew was against a held *connection*, not against a
process: what a warm network needs is to be dialable, so the dial is the wake signal, and being
dialable is what a reservation provides. Cold is the only tier that is not a process. So the
saving is connections and gossip meshes rather than nodes, and a member with thirty conversations
is running thirty-odd of them exactly as `05` §4 says.

**A server out of view is cold rather than warm, which is the conservative half of §7's fourth
question.** This section lists "recent networks" as warm and §7 records that the rule deciding
which ones is unwritten. Until it is, a server is warm only while in view — the reading that runs
fewer nodes rather than one that keeps a set nobody has defined. Conversations are unaffected,
because this section settles that case outright.
