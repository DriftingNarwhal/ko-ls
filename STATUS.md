...
**Gates green as of this date:** 370 tests here, 681 in `../distributed-intranet`, clippy
...
| O4 | **Closed 2026-09-09.** The *replay* half took one ordinary command from 152 ms to 1.5 ms against three hundred channels. The *read* half is now built and switched on: a settled page is **3.6 ms at five hundred records and 6.1 ms at eight thousand**, against 22.8 ms and 397.7 ms for the whole channel — linear, and about five seconds at a hundred thousand, on a control the window re-runs every two seconds. The interface half is `design/09` §4.4: a loaded range that only grows, local pages that load on scroll while the network fetch keeps its button, three top-of-list states told apart, and per-channel `history_incomplete`. **The cursor was the load-bearing fix** — `before: Option<Hlc>` cannot express a page boundary, since two records can share a reading, and a boundary between such a pair excluded both including the one never drawn. Neither field had ever been read, so it was latent rather than shipped | `design/05` §5, `design/09` §4.4 |
