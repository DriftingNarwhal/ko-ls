...
| ~~O17~~ | ~~**The voided-actions report is never read, so a voided revocation restores a member silently.**~~ **Closed 2026-08-23.** Core §2.7.1 point 5 makes the report mandatory *and* names the client's job: watch for your own submitted actions appearing in it and resubmit. `intranet-governance` implements it — `GovernanceLog::reconcile` returns `Reconciliation { canonical, voided, finalized }`, with conformance tests — and `design/05` §3 names the event it surfaces as. `serve` now reconciles whenever governance arrives, splits this member's voided actions from everybody else's, and reports once per heal. The window holds the notice until it is dismissed by hand and asks for it on every refresh, because replay follows the *winning* branch — an action that lost leaves nothing in the projection to find, so a window that missed the event had no other way to learn | Found by reading `design/05` §3 against `kols-api::Event` during the 2026-08-23 audit, not by a failure. Nothing has exercised it because it needs a fork: a partition in which a revocation lands on the losing branch. The window is real rather than theoretical — the spec spells it out, that a member revoked on the losing branch is "a fully current member again on the winning branch, entitled to its epoch key," until somebody notices. Both the design and the spec are already settled, so this is a build gap and not a question | Core §2.7.1 point 5; `design/05` §3, §4; `design/00` §5 |
| ~~O18~~ | ~~**Sidebar order, a network's name and named categories are all specified and unimplemented.**~~ **Closed 2026-08-23.** Spec 07 §1.6 fixes a two-level default order (uncategorised first, categories by position, channels within their category, ties by id, never-positioned last among siblings); §1.7 fixes `chat:network-name`, bounded at 128 bytes and explicitly not an identifier; §1.8 makes a category a named, ordered definition that is metadata over a scope that already exists. §3.8 carries `SetPosition` as channel-update `0x07` and categories as entry kinds `0x05`/`0x06`. All of it is in `kols-core` now: `ChannelChange::SetPosition`, `CategoryChange`, entry kinds `0x05`/`0x06` behind an `EntrySubject` that takes its kind from the discriminant, `ChatPolicy::network_name` with the 128-byte refusal, and `sidebar_order` computing the two-level default. `kols-node` replays both channel positions and category state. 235 tests, clippy clean | Deliberate, and the shape the spec's own header describes: spec 07 is normative, and where it and the implementation differ the divergence is recorded here. Landed as spec first because these were design decisions before they were code, and writing the bytes down twice caught something both times — that `SetPosition` had to be a change rather than a definition field, the alternative re-encoding every channel definition already written; and that ordering channels flatly stops being right the moment categories can be ordered too, which is why §1.6 was rewritten one commit after it landed | spec 07 §1.6, §1.7, §1.8, §3.6, §3.8; `design/00` D31–D35 |
