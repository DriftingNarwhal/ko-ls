...
**Gates green as of this date:** 356 tests here, 681 in `../distributed-intranet`, clippy
...
| O4 | **`kols-store` is built and deliberately not switched on, 2026-09-09.** The *replay* half needed no SQLite and is done — a command re-read, decoded and re-verified every entry file three times over, and one ordinary command went from 152 ms to 1.5 ms against three hundred channels. The *read* half now has its index: schema, the three queries a page needs, the stored rate verdict, and the fingerprint that re-folds a channel when its limits change. `Store::page` reads a page and its acting records rather than the channel. **What remains is not plumbing**: `open_channel` still renders everything, because the window has no paging and rendering a page into an interface that cannot ask for the next one would hide history. That is `design/09`'s question | `design/05` §5 |
...
| `kols-store` | The read-side projection: the schema, the page and target queries, the stored rate verdict and its fingerprint. 11 tests, six of which compare its verdicts against `kols_core::withheld` itself. Not yet read by `open_channel` — see O4 |
