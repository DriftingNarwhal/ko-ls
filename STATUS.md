...
| | |
|---|---|
| **Working on** | Milestone: a client two people on separate networks can use. The window runs a node; join-by-invite in the UI is next |
| **Blocked on** | Nothing |
...
| O6 | **The window has no invite control, no presence, and cannot join by invite** | Joining is `kols join`, and the window has no equivalent — so a person given an invite still needs a terminal. Presence needs the ephemeral gossip of `design/01` §9, which nothing implements. The invite control needs the flow `design/09` §7 carries as an open question | `design/09` §4.1, §5, §7 |
| O7 | **No backup exists.** A seed is written to `<home>/seed` and never surfaced, so a member's only copy is a file — `MasterSeed::to_backup_phrase` exists upstream and nothing calls it | `design/02` §6.3 now says what a backup has to be: a bundle of phrase, network id and a relay address, per network, exported by the client rather than written down. That is a feature to build, not a line to print, which is why `init` currently says the seed file is the only copy rather than pretending otherwise | `design/02` §6.3 |
