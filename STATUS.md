...
| O18 | **Sidebar order, a network's name and named categories are all specified and unimplemented.** Spec 07 §1.6 fixes a two-level default order (uncategorised first, categories by position, channels within their category, ties by id, never-positioned last among siblings); §1.7 fixes `chat:network-name`, bounded at 128 bytes and explicitly not an identifier; §1.8 makes a category a named, ordered definition that is metadata over a scope that already exists. §3.8 carries `SetPosition` as channel-update `0x07` and categories as entry kinds `0x05`/`0x06`. **Half landed.** `ChannelChange::SetPosition` (`0x07`), `ChatPolicy::network_name` with the 128-byte refusal, and `sidebar_order` computing the two-level default are in `kols-core`, with replayed state carrying a position and the boundary passing the change through unaltered. **Categories are not**: there is still no `0x05`/`0x06` entry kind, so folders cannot be named and the sort orders only what channels themselves declare | Deliberate, and the shape the spec's own header describes: spec 07 is normative, and where it and the implementation differ the divergence is recorded here. Landed as spec first because these were design decisions before they were code, and writing the bytes down twice caught something both times — that `SetPosition` had to be a change rather than a definition field, the alternative re-encoding every channel definition already written; and that ordering channels flatly stops being right the moment categories can be ordered too, which is why §1.6 was rewritten one commit after it landed | spec 07 §1.6, §1.7, §1.8, §3.6, §3.8; `design/00` D31–D35 |
