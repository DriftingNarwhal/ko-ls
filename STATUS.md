...
| O4 | **Half built 2026-09-08, and the halves wanted different things.** The *replay* half needed no SQLite: a command re-read, decoded and re-verified every entry file three times over, and the store now holds the log and its folds across calls — one ordinary command went from 152 ms to 1.5 ms against three hundred channels, and a settled send is near flat. The *read* half remains and is larger: `open_channel` reads every record in the channel whatever `limit` says — 222 ms at six thousand records, about four seconds at a hundred thousand — and a page needs records in an HLC range plus whatever targets them, which is a query rather than a fold. **That is what `kols-store` is for** | `design/05` §5 |
